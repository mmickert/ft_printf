...
/*   Updated: 2023/10/25 13:35:41 by mickert          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (format[i] == 'c')
		return (ft_putchar_fd((char)va_arg(args, int), 1, count));
	else if (format[i] == 's')
		count = ft_putstr_fd(va_arg(args, char *), 1, count);
	else if (format[i] == 'p')
		count = ft_putpointer_fd(va_arg(args, size_t), 1, count);
	else if (format[i] == 'i' || format[i] == 'd')
		count = ft_putnbr_fd(va_arg(args, int), 1, count);
	else if (format[i] == 'u')
